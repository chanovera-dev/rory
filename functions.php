 ...
 *
 * This file acts as the engine of the theme, handling:
 * - Theme setup and support (menus, images, HTML5)
 * - Enqueuing of scripts and styles with versioning
 * - Custom Gutenberg block rendering and filtering
 * - Customizer settings for site-specific data
...
function setup_rory()
{
    ...
    register_nav_menus(
        array(
            'primary' => __('Main Menu', 'rory'),
            'social' => __('Social Menu', 'rory'),
            'footer-1' => __('Footer 1', 'rory'),
            'footer-2' => __('Footer 2', 'rory'),
            'footer-3' => __('Footer 3', 'rory'),
        )
    );
 ...
 */
function rory_enqueue_style($handle, $path, $media = 'all')
{
 ...
 */
function rory_enqueue_script($handle, $path)
{
    $uri = get_template_directory_uri();
...
function footer_components()
{
    ...
    rory_enqueue_style('wp-root', $a['css']['wp-root']);
    rory_enqueue_style('custom-forms', $a['css']['custom-forms']);
    rory_enqueue_style('shapes', $a['css']['shapes']);
    rory_enqueue_style('normalize', $a['css']['normalize']);
    rory_enqueue_script('global-script', $a['js']['global-script']);

    if (is_user_logged_in()) {
        rory_enqueue_style('wp-logged-in', $a['css']['wp-logged-in']);
    }
}
...
{
    $separator = rory_get_icon('separator');
    $icon_home = rory_get_icon('home');
    $home = 'Inicio';
    $showCurrent = 1;
    $showOnHome = 0;
    $current = '';
    ...
    elseif (is_archive()) {
        ...
        if ($paged === 1) {
            echo '<h1 class="page-title">' . esc_html__('Contenido más reciente', 'rory') . '</h1>';
        } else {
        ...
        if ($paged === 1) {
            ...
            esc_html_e('Resultados de búsqueda de "', 'rory');
        ...
        if (get_post_type() != 'post') {
            ...
            if ($categories) {
                ...
                foreach ($categories as $category) {
                    ...
                    foreach ($ancestors as $ancestor_id) {
                        ...
                        if ($ancestor) {
                            $links[] = '<a class="post-tag" href="' . esc_url(get_category_link($ancestor->term_id)) . '">' . rory_get_icon('category') . esc_html($ancestor->name) . '</a>';
                        }
                    }

                    $links[] = '<a class="post-tag" href="' . esc_url(get_category_link($category->term_id)) . '">' . rory_get_icon('category') . esc_html($category->name) . '</a>';
                }
            ...
            if ($post_format) {
                ...
                switch ($post_format) {
                    case 'aside':
                        $format_label = __('Minientrada', 'rory');
                        break;
                    case 'gallery':
                        $format_label = __('Galería', 'rory');
                        break;
                    case 'image':
                        $format_label = __('Dibujo', 'rory');
                        break;
                    case 'video':
                        $format_label = __('Video', 'rory');
                        break;
                    case 'quote':
                        $format_label = __('Cita', 'rory');
                        break;
                    case 'link':
                        $format_label = __('Artículo externo', 'rory');
                        break;
                }

                if ($format_label) {
                    echo '<a class="post-tag" href="' . esc_url(get_post_format_link($post_format)) . '">' . rory_get_icon($post_format) . esc_html($format_label) . '</a>';
                }
            }
...
function custom_render_gallery_block($block_content, $block)
{
    ...
    <div class="post-gallery-wrapper">
        ...
        <div class="post-gallery-thumbs-container">
            <button class="btn-pagination" aria-label="Anterior"><?= rory_get_icon('backward'); ?></button>
            <div class="post-gallery-thumbs"></div>
            <button class="btn-pagination" aria-label="Siguiente"><?= rory_get_icon('forward'); ?></button>
        </div>
    </div>
...
function custom_wp_block_search($block_content, $block)
{
    if ($block['blockName'] === 'core/search') {
        ob_start();
        ?>
        <form role="search" method="get" action="<?php echo home_url('/'); ?>" class="rory-block-search">
            <div class="rory-block-search__inside-wrapper ">
                <input class="rory-block-search__input" id="rory-block-search__input-<?php echo $block['clientId']; ?>"
                    placeholder="<?php esc_html_e('Buscar', 'rory'); ?>" value="" type="search" name="s" required="">
                <button aria-label="<?php esc_html_e('Buscar', 'rory'); ?>"
                    class="rory-block-search__button rory-element-button" type="submit">
...
function custom_category_list($output, $args)
{
    ...
    $output = '';
    foreach ($categories as $category) {
        $svg_icon = rory_get_icon('category');
 ...
 */
function rory_has_related_posts($post_id = null)
{
    if (!$post_id) {
        $post_id = get_the_ID();
    }
...
function posts_styles()
{
    if (is_home() or is_archive() or is_search()) {
        ...
        rory_enqueue_script('animate-in', $a['js']['animate-in']);
        rory_enqueue_script('posts-scripts', $a['js']['posts-scripts']);

        if (is_active_sidebar('sidebar-1')) {
            rory_enqueue_style('sidebar', $a['css']['sidebar']);
        }
    }
}
...
function page_template()
{
    ...
    if (is_page() or is_single()) {
        ...
        rory_enqueue_style('page', $a['css']['page']);
        rory_enqueue_style('breadcrumbs', $a['css']['breadcrumbs']);
        ...
        if ($post && (has_block('core/gallery', $post) || has_shortcode($post->post_content, 'gallery'))) {
            rory_enqueue_style('post-gallery-styles', $a['css']['post-gallery-styles']);
            rory_enqueue_script('post-gallery-script', $a['js']['post-gallery-script']);
        }

        if (is_single()) {
            rory_enqueue_style('single', $a['css']['single']);
            if (rory_has_related_posts()) {
                rory_enqueue_style('related-styles', $a['css']['related-styles']);
                rory_enqueue_script('related-script', $a['js']['related-script']);
                rory_enqueue_script('animate-in', $a['js']['animate-in']);
                rory_enqueue_script('post-scripts', $a['js']['post-scripts']);
            }

            if (comments_open()) {
                rory_enqueue_style('custom-comments', $a['css']['comments']);
            }
        }
    }
}
...
function page404_styles()
{
    if (is_404()) {
        $a = rory_get_assets();
        rory_enqueue_style('error404', $a['css']['error404']);
    }
}
